...
**Version:** 1.5 | **Date:** 2026-04-04 | **Status:** Live / Active Development
...
- Copyright line: `© [year] Cygnus Consulting. All Rights Reserved. · Built & maintained by Neuraflow`
  - "Neuraflow" links to `https://neuraflow.cloud/` (opens in new tab, `rel="noopener noreferrer"`, styled `text-brand-accent`)
...
### 11.1 Hosting
- **Platform:** Netlify (primary) / cPanel Apache (production at cygnus.co.ke)
- **Build command:** `npm run build`
- **Publish dir:** `dist/`
- **SPA redirect (Netlify):** `/* → /index.html 200` (configured in `netlify.toml`)
- **SPA redirect (Apache/cPanel):** `dist/.htaccess` — must be manually written after each build before uploading. Content:
  ```apache
  Options -MultiViews
  RewriteEngine On
  Redirect 301 /nca-cpd-training/ /training/nca
  Redirect 301 /nca-cpd-training /training/nca
  RewriteCond %{REQUEST_FILENAME} !-f
  RewriteRule ^ index.html [QL]
  ```
- **Note:** `.htaccess` is NOT auto-generated by `npm run build` — create it manually in `dist/` before zipping and uploading to cPanel.
...
**Interface (updated v1.5):**
```ts
interface NCASession {
  id:        number;
  date:      string;      // display range, e.g. "25–26 March 2026"
...
| Copyright | © [year] Cygnus Consulting. All Rights Reserved. · Built & maintained by [Neuraflow](https://neuraflow.cloud/) |
...
|---|---|---|---|
| 1.5 | 2026-04-04 | — | NCA sessions → 2-day format; CPD → 20; price → KES 5,000; calendar shows day sub-topics (§11d); Apache .htaccess SPA fix + /nca-cpd-training/ 301 redirect (§11.1); footer Neuraflow credit (§5.10, §15.11) |
